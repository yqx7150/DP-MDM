...
import time
from utils import get_psnr_is_declining
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model, model1, model2, test_data, img_name, save_path):
    """ The PC sampler funciton.
    Args:
      model: score model.
      model1: score model (mask : 50x50).
      model2: score model (mask : 120x120).
    Returns:
    ...
    with torch.no_grad():
      ...
      psnr_is_declining = get_psnr_is_declining()
      ...
      mask_square_1 = np.zeros((coil,256,256))
      mask_square_2 = np.zeros((coil, 256, 256))
      # mask_item = io.loadmat('./parallel_inputdata/contract_mask/poisson/r6.mat')['mask']
      mask_item = io.loadmat('./datasets/GMX/compare_exp/Gyu_random/random2D_acs24_r15.mat')['mask']
      mask_square_item1 = io.loadmat('datasets/GMX/GMX_multi_data_and_mask/train_square/idea1_Square/Square_mask7_20.mat')['mask']
      mask_square_item2 = io.loadmat('datasets/GMX/GMX_multi_data_and_mask/train_square/idea1_Square/Square_mask7_50.mat')['mask']
      ...
      for i in range(coil):
        mask[i,:,:] = mask_item
        mask_square_1[i,:,:] = mask_square_item1
        mask_square_2[i ,:, :] = mask_square_item2
      ...
      write_images(abs(mask_square_item1), osp.join(save_path, img_name + 'mask_1' + '.png'))
      write_images(abs(mask_square_item2), osp.join(save_path, img_name + 'mask_2' + '.png'))
      ...
      for indx in range(sde.N):
        ...
        #========================================================== Sake 1
        rec_Image_2, Krec_w2 = sake(k_complex2, Ksample, mask, ksize, wnthresh, sakeIter, ori_data)

        Krec_w2_t = Krec_w2.transpose(2,0,1)

        # used for padding
        k_pc1_copy = np.copy(Krec_w2_t)
        ...
        for ii in range(coil):
          Krec_w2_t_m[ii,:,:] = np.multiply(mask_square_1[ii,:,:], Krec_w2_t[ii,:,:])#add mask1
        ...
        for i in range(coil):       
          # k_complex[i,:,:] = wgt2k(k_w[i,:,:],weight[i,:,:],Ksample[i,:,:])
          k_w_m[i, :, :] = k_w_m[i, :, :] + np.multiply(1 - mask_square_1[i, :, :], k_pc1_copy[i, :, :])
        ...
        for i in range(coil): 
          k_w_m_mask[i, :, :] = np.multiply(mask_square_1[i, :, :], k_w_m_mask[i, :, :]) #add mask1
        ...
        Krec_w2_t_m = np.zeros((coil, 256, 256), dtype=np.complex64)
        for ii in range(coil):
          # TODO mask_square -> 120x120
          Krec_w2_t_m[ii,:,:] = np.multiply(mask_square_2[ii,:,:], Krec_w2_t[ii,:,:])#add mask2
          x_input_m[ii,0,:,:] = np.real(Krec_w2_t_m[ii,:,:])
          x_input_m[ii,1,:,:] = np.imag(Krec_w2_t_m[ii,:,:])
          x_input_m[ii,2,:,:] = np.real(Krec_w2_t_m[ii,:,:])
          x_input_m[ii,3,:,:] = np.imag(Krec_w2_t_m[ii,:,:])
        ...
        x, x_mean_m = predictor_update_fn(x_mean_m, vec_t, model=model2)
        
        x_mean_m = x_mean_m.cpu().numpy() # (12,6,256,256)
        x_mean_m = np.array(x_mean_m,dtype=np.float32) 
        ...
        for i in range(coil):       
          # k_complex[i,:,:] = wgt2k(k_w[i,:,:],weight[i,:,:],Ksample[i,:,:])
          k_w_m[i, :, :] = k_w_m[i, :, :] + np.multiply(1 - mask_square_2[i, :, :], k_pc1_copy[i, :, :])#填充
          k_complex2_m[i,:,:] = Ksample[i,:,:] + k_w_m[i,:,:]*(1-mask[i,:,:])
        ...
        for i in range(coil): 
          # TODO no weight
          k_w_m_mask[i,:,:] = k_complex2_m[i,:,:]
          # k_w_m_mask[i,:,:] = k2wgt(k_complex2_m[i,:,:],weight[i,:,:])
          k_w_m_mask[i, :, :] = np.multiply(mask_square_2[i, :, :], k_w_m_mask[i, :, :])#add mask2
          x_input_m[i,0,:,:] = np.real(k_w_m_mask[i,:,:])
          x_input_m[i,1,:,:] = np.imag(k_w_m_mask[i,:,:])
          x_input_m[i,2,:,:] = np.real(k_w_m_mask[i,:,:])
          x_input_m[i,3,:,:] = np.imag(k_w_m_mask[i,:,:])
        ...
        x6 = x_mean_m
        ##======================================================= Corrector 3
        x4,x5,x6,x_mean_m = corrector_update_fn(x4,x5,x6,x_mean_m, vec_t, model=model2)    
        x_mean_m = x_mean_m.cpu().numpy() 
        x_mean_m = np.array(x_mean_m,dtype=np.float32)
        ...
        for i in range(coil):
          # TODO no weight
          k_w_m[i, :, :] = k_w_m[i, :, :]     
          # k_w_m[i, :, :] = wgt2k(k_w_m[i, :, :], weight[i, :, :], Ksample[i, :, :])          
          k_w_m[i, :, :] = k_w_m[i, :, :] + np.multiply(1 - mask_square_2[i, :, :], k_pc1_copy[i, :, :])#填充
          k_complex2_m[i,:,:] = Ksample[i,:,:] + k_w_m[i,:,:]*(1-mask[i,:,:])
          rec_Image_m[i,:,:] = np.fft.ifft2(k_complex2_m[i,:,:])
        ...
        for i in range(coil): 
          ...
          k_mask[i,:,:] = np.multiply(mask_square_2[i,:,:],x_w_dc_kdata[i,:,:])
